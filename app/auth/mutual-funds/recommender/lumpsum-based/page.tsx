...
import { recommendationLumpSumFormType, recommendLumpSumSchema, recommendSchema } from "@/lib/auth-form-schemas";
import { z } from "zod";
...
export default function RecommendPage() {
    ...
    const recommendationForm = useForm<recommendationLumpSumFormType>({
        resolver: zodResolver(recommendLumpSumSchema),
        defaultValues: {
            min_lumpsum: "",
            required_return: "",
            category: "",
    ...
    async function onSubmit(values: recommendationLumpSumFormType) {
        startTransition(async () => {
            try {
                const validatedData = recommendLumpSumSchema.safeParse(values);
                if (validatedData.error || !validatedData.data) {
                    toast.error("Failed to validate");
                    return;
                }
                const url: string = `${process.env.NEXT_PUBLIC_BACKEND_URL}/recommend/lumpsum`

                const response = await axios.post(
                    url,
                    {
                        min_sip: parseFloat(validatedData.data.min_lumpsum),
                    ...
                    <Form {...recommendationForm}>
                        ...
                        >
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>Minimum LumpSum</FormLabel>
                                        <FormControl>
